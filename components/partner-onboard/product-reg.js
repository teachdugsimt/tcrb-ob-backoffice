...
const ProductReg = inject('partnerOnboard')(observer((props) => {
  ...
  const [tmpGrantProduct, setTmpGrantProduct] = useState(null)
  const { t, partnerOnboard } = props

  useEffect(() => {
    if (partnerOnboard.dataGrantProduct && partnerOnboard.dataGrantProduct != null) {
      partnerOnboard.getDataPartnerProductList(JSON.parse(JSON.stringify(partnerOnboard.tmp_partner_id)))
    }
  }, [partnerOnboard.tmp_partner_id, partnerOnboard.dataGrantProduct])
